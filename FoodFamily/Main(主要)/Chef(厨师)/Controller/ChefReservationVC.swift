...
class ChefReservationVC: UIViewController,UITableViewDelegate,UITableViewDataSource,FoodSelectNumberReservationsDelegate,FoodSelectNumberPeopleDelegate,UITextFieldDelegate {
    ...
    @objc func nextOnClick(){
        let vc = ChefReservationDetailsVC()
    ...
    func checkInpunt()->Bool{
        if reservationsNumber.count == 0 && reservationsTime.count == 0 && !Tools.validateMobile(mobile: phoneTextField.text!) && !Tools.validateCode(code: codeTextField.text!) {
            SVProgressHUD.showInfo(withStatus: "输入不能为空")
            return false
        }
        
        if reservationsNumber.count == 0 {
            SVProgressHUD.showInfo(withStatus: "请输入预约人数")
            return false
        }
        
        if reservationsTime.count == 0 {
